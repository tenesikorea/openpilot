...
void AnnotatedCameraWidget::drawCgear(QPainter &p) {
  p.save();
  const SubMaster &sm = *(uiState()->sm);
  auto car_state = sm["carState"].getCarState();
  ...
  p.setFont(InterFont(200, QFont::Normal));
...
void AnnotatedCameraWidget::drawBsd(QPainter &p) {
  p.save();
  const SubMaster &sm = *(uiState()->sm);
  auto car_state = sm["carState"].getCarState();

  const int position_x = 900; //X축 위치
  const int car_y = 625; // Y축 위치
...
void AnnotatedCameraWidget::drawErpm(QPainter &p) { // rpm정보 - 함수선언
  p.save();
  const SubMaster &sm = *(uiState()->sm);
  auto car_state = sm["carState"].getCarState();
