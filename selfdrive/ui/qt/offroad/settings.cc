...
TenesiCarPanel::TenesiCarPanel(QWidget* parent) : QWidget(parent) {
  ...
  layout->addWidget(new ParamControl("Tenesi_gap_enable"," e2e acc 가감속 변경모드 "," e2e acc 가감속 변경모드  사용하기", "../assets/offroad/icon_shell.png", this));
  layout->addWidget(horizontal_line()); // 구분선 구간
  //layout->addWidget(new ParamControl("Tenesi_path_cost_enable","경로 게산 코스트 적용선택","경로 게산 코스트를 사용하기", "../assets/offroad/icon_shell.png", this));
  //layout->addWidget(new CValueControl("Tenesi_path_cost", "경로 게산 코스트", "경로 게산 코스트. 단위 *0.01", "../assets/offroad/icon_road.png", 90, 150, 1));
  layout->addWidget(new ParamControl("Tenesi_dz_cost_enable","e2e 데인져존 코스트 적용선택","e2e 데인져존 코스트를 사용하기", "../assets/offroad/icon_shell.png", this));
  layout->addWidget(new CValueControl("Tenesi_dz_cost", "e2e 데인져존 코스트", "e2e 가감속 데인져존 코스트.", "../assets/offroad/icon_road.png", 50, 100, 1));
  layout->addWidget(horizontal_line()); // 구분선 구간
