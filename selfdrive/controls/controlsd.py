...
SR_SCALE_V = [16.5, 16.5, 16.5, 16.5, 17.2, 17.2, 17.2, 17.2, 16.5, 16.5, 16.5, 16.8, 16.8, 16.8, 16.8, 16.8, 16.8, 15.5, 15.2, 15.2, 15.2, 15.2, 15.2, 15.2, 15.2, 15.2, 15.2]
...
class Controls:
  ...
  def update_events(self, CS):
    ...
    #if not self.sm['liveLocationKalman'].inputsOK and not NOSENSOR: # 로컬라이저 에러가 너무 자주 발생
    #  self.events.add(EventName.localizerMalfunction) # 로컬라이저 에러가 너무 자주 발생
    ...
    if not REPLAY:
      ...
      cruise_mismatch = CS.cruiseState.enabled and (not self.enabled) # or not self.CP.pcmCruise) //Neokii
    ...
    #if planner_fcw or model_fcw: //Neokii
    #  self.events.add(EventName.fcw) //Neokii
  ...
  def state_transition(self, CS):
    ...
    #self.v_cruise_helper.update_v_cruise(CS, self.enabled, self.is_metric) //Neokii
    ...
    # DISABLED
      ...
      if self.events.contains(ET.ENABLE):
        if self.events.contains(ET.NO_ENTRY):
          ...
          #self.v_cruise_helper.initialize_v_cruise(CS, self.experimental_mode) //Neokii
  ...
  def state_control(self, CS):
    ...
    #sr = max(lp.steerRatio, 0.1)  //Neokii
    ...
    if Params().get_bool('Steer_SRTune'):
      sr_v = float(int(Params().get("Steer_SRTune_v", encoding="utf8"))) * 0.01 # 가변SR 선택시에 SR비율 조절
      sr = interp(CS.vEgo * 3.6, SR_SCALE_BP, SR_SCALE_V) * sr_v # Sr비율조절
    ...
    CC.longActive = self.enabled and not self.events.contains(ET.OVERRIDE_LONGITUDINAL) and self.CP.openpilotLongitudinalControl \
                    and CS.cruiseState.enabled # //Neokii
  ...
  def publish_logs(self, CS, start_time, CC, lac_log):
    ...
    self.speed_controller.inject_events(CS, self.events) # //Neokii
