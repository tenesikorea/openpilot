...
class NoEntryAlert(Alert):
  def __init__(self, alert_text_2: str,
               alert_text_1: str = "펭수 뿔딱중! 참치를 주세요!",
...
def calibration_incomplete_alert(CP: car.CarParams, CS: car.CarState, sm: messaging.SubMaster, metric: bool, soft_disable_time: int) -> Alert:
  first_word = '2차 캘리브레이션' if sm['liveCalibration'].calStatus == log.LiveCalibrationData.Status.recalibrating else '1차 캘리브레이션'
  return Alert(
...
EVENTS: Dict[int, Dict[str, Union[Alert, AlertCallbackType]]] = {
  ...
  EventName.reverseGear: {
    ...
    ET.USER_DISABLE: SoftDisableAlert("후진 [R] 상태"),
  ...
  EventName.plannerError: {
    ET.IMMEDIATE_DISABLE: SoftDisableAlert("플래너 솔루션 오류"),
