...
class CruiseStateManager:
  ...
  def update(self, CS, main_buttons, cruise_buttons, buttons_dict, available=-1, cruise_state_control=True):
    ...
    if ascc_auto_set and self.cruise_auto and not self.enabled: # 캔슬버튼을 누르지 않았다면...초기 false상태에서 오토셋은 작동한다..
      self.enabled = True #                  오토셋 작동했다는 조건을 True로 만들어준다..
  ...
  def update_cruise_state(self, CS, v_cruise_kph, btn):
    ...
    if Params().get_bool("Tenesi_gap_enable"):
      if btn == ButtonType.gapAdjustCruise: #btn == ButtonType.gapAdjustCruise and self.btn_long_pressed:
        if (Params().get_bool("ExperimentalMode")):
          Params().put("ExperimentalMode", "0")
        else:
