...
if __name__ == "__main__":
  ...
  uds_client = UdsClient(panda, 0x7D0, bus=args.bus, debug=args.debug) # 7d4는 mdps

  print("\n[START DIAGNOSTIC SESSION]") # 모듈등이 연결이 가능하다면 UDS를 이용해서 접속시도를 한다..
  session_type : SESSION_TYPE = 0x07 # type: ignore # 기존의 세션타입은 1-4까지 이지만.. 07 이라는 모드로 진입
  uds_client.diagnostic_session_control(session_type) # 서비스 모드를 ..
  # SERVICE_TYPE 타입에 diagnostic_session_control함수는 0x10(SERVICE_TYPE)이용한 명령..session_type을 07로 설정해서 1회성 작업시작

  print("[HARDWARE/SOFTWARE VERSION]")
  fw_version_data_id : DATA_IDENTIFIER_TYPE = 0xf100 # type: ignore # F100이라는 임의의 명령으로 펌웨어 버젼을 읽을수 잇다..
  fw_version = uds_client.read_data_by_identifier(fw_version_data_id) # 0x22 서비스SERVICE_TYPE을 이용해서 반응을 살핀다..
  print(fw_version)
  if fw_version not in SUPPORTED_FW_VERSIONS.keys(): # 펌웨어 리스트에 없다면 모든 작업을 종료한다..
    print("radar not supported! (aborted)")
    sys.exit(1)

  print("[GET CONFIGURATION]")
  config_data_id : DATA_IDENTIFIER_TYPE = 0x0142 # type: ignore #0142라는 것으로 요청..
