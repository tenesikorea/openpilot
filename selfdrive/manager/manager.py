...
#    ("NewRadarInterface", "0"),
    ...
    ("Steer_SRTune_v", "92"),  # 가변 조향비율 초기값 수정
    ("Lane_Change", "70"),  # DH기준 기본 차속변경속도 70으로 수정
    ...
    ("Sound_Mdpserr", "1"),  # mdps에러시 음성듣기
    ("TenesiBrakeSelect", "3"),  # 브레이킹작동시 허드에 표시하기
    ("TenesiSccStopCost", "30"),  # vEgoStopping 기본값 0.3를 변경하기..
    ("Tenesi_Gas_Factor", "10"),  # Scc스무서 상태에서 가속이 더디어서 셋팅
    ("SccOnBus2", "1"),  # SCC 2번 개조상태 기본적용
    ("CruiseStateControl", "1"),  # SCC 2번 개조상태 기본적용
    ("Tenesi_dz_cost", "50"),  # DANGER_ZONE_COST lont_mpc.py e2e가감속 계산 코스트
    ("Tenesi_dz_cost_enable", "0"),  # DANGER_ZONE_COST lont_mpc.py e2e가감속 계산 코스트
    #("Tenesi_path_cost", "100"),  # DANGER_ZONE_COST lont_mpc.py e2e가감속 계산 코스트
    #("Tenesi_path_cost_enable", "0"),  # DANGER_ZONE_COST lont_mpc.py e2e가감속 계산 코스트
    ("Tenesi_gap_enable", "0"),  #  e2e acc 가감속 변경모드
  ]
  if not PC:
