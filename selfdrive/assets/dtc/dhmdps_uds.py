...
class CanClient():
  ...
  def _recv_filter(self, bus: int, addr: int) -> bool:
    ...
    if self.tx_addr == 0x7D4: # Tenesi Genesis MDPS Hard Fault Clear Run Command
      is_response = addr >= 0x7DC and addr <= 0x7DC # Tenesi Genesis MDPS Hard Fault Clear Run Command
      if is_response:
        if self.debug:
...
class UdsClient():
  ...
  def diagnostic_session_control(self, session_type: SESSION_TYPE):
    self._uds_request(SERVICE_TYPE.DIAGNOSTIC_SESSION_CONTROL, subfunction=0x90) # Tenesi Genesis MDPS Hard Fault Clear Run Command
  ...
  def clear_diagnostic_information(self, dtc_group_type: DTC_GROUP_TYPE):
    data = struct.pack('!I', dtc_group_type)[0:]  # 3 bytes
    self._uds_request(SERVICE_TYPE.CLEAR_DIAGNOSTIC_INFORMATION, subfunction=0x40) # Tenesi Genesis MDPS Hard Fault Clear Run Command
