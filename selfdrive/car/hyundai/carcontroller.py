...
from openpilot.common.params import Params, put_nonblocking # 테네시 추가
  ...
  def update(self, CC, CS, now_nanos):
    ...
    if self.CP.carFingerprint in CANFD_CAR:
      ...
      if self.frame % 2 == 0 and self.CP.openpilotLongitudinalControl:
        ...
        # 롱컨일때 가속값 튜닝
        creepspeedstart = float(int(Params().get("Creep_Speed_Start", encoding="utf8")))
        if CS.out.vEgo < creepspeedstart:  # 시속5키로까지만 0.05를 적용..차가 튀어 나가는 느낌을 줄인다..
        ...
        if CC.longActive:
          ...
          tenesigasfactor = float(int(Params().get("Tenesi_Gas_Factor", encoding="utf8"))) * 0.01
          tenesigasfactor = tenesigasfactor + accel # 크루즈시 가속추가
          if CS.out.vEgo > creepspeedstart:
            accel = tenesigasfactor
