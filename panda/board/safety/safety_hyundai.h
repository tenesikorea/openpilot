...
const CanMsg HYUNDAI_TX_MSGS[] = {
  {593, 2, 8},                              // MDPS12, Bus 2  //Neokii
  {832, 0, 8},                              // LKAS11, Bus 0
  {1056, 0, 8},                             // SCC11, Bus 0 //Neokii
  {1057, 0, 8},                             // SCC12, Bus 0 //Neokii
  {1290, 0, 8},                             // SCC13, Bus 0 //Neokii
  {905, 0, 8},                              // SCC14, Bus 0 //Neokii
  {909, 0, 8},                              // FCA11 Bus 0 //Neokii
  {1155, 0, 8},                             // FCA12 Bus 0 //Neokii
  {1157, 0, 4},                             // LFAHDA_MFC, Bus 0
  {1186, 0, 8},                             // FRT_RADAR11, Bus 0 //Neokii
  {1265, 0, 4}, {1265, 2, 4},               // CLU11, Bus 0, 2 //Neokii
};
...
static int hyundai_rx_hook(CANPacket_t *to_push) {
  ...
  if (valid && (addr == 1056)) { //  MainMode_ACC  //Neokii
    // 1 bits: 0
    int cruise_available = GET_BIT(to_push, 0U);
    hyundai_common_cruise_state_check(cruise_available); //Neokii
  }
  ...
  if (valid && (bus == 0)) {
    ...
    gas_pressed = brake_pressed = false; //Neokii
...
uint32_t last_ts_lkas11_from_op = 0; //Neokii
uint32_t last_ts_scc12_from_op = 0; //Neokii
uint32_t last_ts_mdps12_from_op = 0; //Neokii
uint32_t last_ts_fca11_from_op = 0; //Neokii
...
static int hyundai_fwd_hook(int bus_num, int addr) {
  ...
  uint32_t now = microsecond_timer_get(); //Neokii
